...
class GAT(nn.Module):
    def __init__(self, in_size, hid_size, out_size, num_layers, num_heads=2, dropout=0.5):
        ...
        # Define GAT layers based on the number of layers
        if num_layers == 1:
            self.layers.append(
                dglnn.GATConv(
    ...
    def forward(self, blocks, x):
        ...
        for l, (layer, block) in enumerate(zip(self.layers, blocks)):
            h = layer(block, h)
            # GATConv returns [N, num_heads, feat_size], need to reshape if not last layer
            if l != len(self.layers) - 1:
                h = h.flatten(1)  # Concatenate heads
                h = F.relu(h)
                h = self.dropout(h)
            else:
                h = h.mean(1)  # Average heads for output
        return h
