...
def get_accuracy_movies(eval_output, path,save=True):
...
def get_accuracy_reddits(eval_output, path):
...
def get_accuracy_products(eval_output, path):
    ...
    return correct/len(df)
def get_accuracy_lp(eval_output, path,save=True):
...
eval_funcs = {
    ...
    'Reddits': get_accuracy_reddits,
    '4_Arts_Crafts_and_Sewing':get_accuracy_movies,
    "9_CDs_and_Vinyl":get_accuracy_movies,
}
