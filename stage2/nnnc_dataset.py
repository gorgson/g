...
class NodeClassificationDataset(object):
    def __init__(self, root: str,verbose: bool=True, device: str="cpu",bert_name: str = "bert-base-uncased",feat="clip",data_path="",save=False,trun=True):
        ...
        self.label_to_second_category=self.map

        all_categories=list(set(graph["subcategory"].values()))
        ...
        self.graph.ndata['image_feat'] = torch.load(os.path.join(root,"cimg_feat.pt")).to(self.device)[:,0].contiguous()
        self.graph.ndata["text_feat"]=node_ids.to(self.device)[:,0].contiguous()
